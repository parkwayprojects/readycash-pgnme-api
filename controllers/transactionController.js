...
const { today, merge } = require('../utils/helpers')


const fromInseption = async (req, res) => {
  const { mainAccId, incomeAccId } = req.body
  const mainTransPromise =  db(accountTransactionListQuery(mainAccId))
  const incomeListPromise =  db(incomeAccountQuery(incomeAccId))
